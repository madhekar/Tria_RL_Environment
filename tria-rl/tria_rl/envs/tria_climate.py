...
class TriaClimateEnv(gym.Env):
    ...
    metadata = {
                ...
                'reward_scaling' : [-1,-1,-.1],#[-1, -1, -0.03],#[-0.09, -0.09, -0.0099],#[-0.9, -0.9, -0.0375], #[-0.9, -0.9, -0.05],#[-.05,-.05,-.03],
    ...
    def reset(self, seed=1234, options=None):
        ...
        if self.metadata['test'] == 0:
          self.state = [np.random.randint(self.metadata['t_min'], self.metadata['t_max']),
            ...
            self.state = random.choice([[60,40,0], [80,60,400],[70, 50, 200], [90, 70, 410], [50, 30, -10]])#random.choice([[65,40,0], [80,60,200], [50,30,201], [85,70,500], [40,20,501],[90,80,800]])#random.choice([[81, 61, 201],[-50,0,0],[120,100,2000],[70,50,100 ]])#[81, 61, 201] #
        self.equilibrium_cycles = self.metadata['equilibrium_cycles']
